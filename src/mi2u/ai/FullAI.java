...
public class FullAI extends AIController{
    ...
    public class LogicMode extends Mode{
        ...
        public boolean printUI(PrintI inst){
            //format: UI.type(var)
            var text = inst.value;
            var str = text.isobj && inst.value.numi() != 0 ? PrintI.toString(text.objval) : String.valueOf(text.numval);
            ...
            for(int i = 0; i < exec.vars.length; i++){
                LVar var = exec.vars[i];

                if(var.name.equals(targetName)){
                    if(customAIUITable.getChildren().size > 60) return false;//no too many uis
                    switch(type){
                        case "button" -> customAIUITable.button(targetName, textbtoggle, () -> var.setbool(!var.bool())).update(tb -> tb.setChecked(var.bool()));
                        case "field" -> customAIUITable.field(String.valueOf(var.num()), TextField.TextFieldFilter.floatsOnly, s -> var.setnum(Strings.parseDouble(s, 0)));
                        case "choose" -> {
                            if(var.isobj && !(var.obj() instanceof MappableContent)) return false;
                            customAIUITable.button(targetName, textb, () -> {
                                ...
                                buildTable(chooseContentTable, new Seq<UnlockableContent>().add(content.items()).add(content.liquids()).add(content.statusEffects()).add(content.blocks()).add(content.units()), () -> var.obj() instanceof UnlockableContent uc ? uc : null, content -> var.setobj(content), false, 8, 8);
                        ...
                        case "info" -> {
                            ...
                            customAIUITable.add("").fill().with(b -> {
                                ...
                                b.update(() -> {
                                    b.setText(PrintI.toString(var.obj()));
                                    b.setColor(b.cullable ? Color.cyan : Color.white);
                                    if(b.hasMouse()) HoverTopTable.hoverInfo.setHovered(var.obj());
                                    if(b.cullable){
                                        if(var.obj() instanceof Posc posc && control.input instanceof InputOverwrite ipo){
